...
Besonders auffällig sind die Zeitunterschiede zwischen den Abfragen der simplen Zahlen (COUNT) und dem Abfragen konkreter Daten, wie \textit{ids} oder \textit{response rates}. Hier gibt es jedoch keinen Unterschied beim Microservice? Warum ist das so? Weil hier schlichtweg das gleiche passiert. Es werden hier alle Schritte ausgeführt, bevor die Daten bereitgestellt werden. Es muss jedoch darauf hingewiesen werden, dass im Microservice hier nie so große Datenmengen aus der Datenbank geholt werden wie in MongoDB. Das liegt einfach an der Zufälligkeit der Daten. Im Propheten wurden hier maximal rund 20.000 Felder gepluckt. Was für den Anwendungsfall durchaus realistisch, aber im Vergleich einfach nicht fair ist. Die letzte Zeile der Tabelle zeigt hier den direkten Vergleich beim Abfragen aller User IDs. Hier fällt die extreme Laufzeit des Microservices auf. Es ist nicht ganz klar, worauf dies zurück zu führen ist.
...
Weiterhin ist nicht ganz klar, warum es überhaupt zu solch extremen Zeiten kommt. Dies ist vermutlich eher ein Problem im Code, als bei der Datenbankabfrage an sich. Dies ist vermutlich zu beheben.
Nichtsdestotrotz hat man hier immer hohe Zeiten, bei einer Requestlaufzeit von 100ms, wird hier im ersten Schritt immer ein Fehler auftreten. Also wären hier immer mehrere Requests notwendig. Ggf. bietet es sich an, hier einen initialen Timeout, auf Basis der durchschnittlichen Berechnungslaufzeit, einzurichtetn um unnötige Requests zu sparen.
